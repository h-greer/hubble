...
class PointResolvedFit(ModelFit):
    ...
    def loglike(self, model, exposure, per_pix=False, return_im=False):
        ...
        if "resolved" in model.params.keys():
            ...
            return super().loglike(model, exposure, per_pix=per_pix, return_im=return_im) + 1.* L2_loss(dist) + 1.* ME_loss(dist) +  2*TSV_loss(dist)
        #+ 0.05* L1_loss(dist) +  0.1*TV_loss(dist)
...
# %%
flat, _ = jax.flatten_util.ravel_pytree(dict(sorted((flat_fn(C) | {"resolved": {"N458_F160W": np.ones(wid**2 * oversample**2)}}).items())))
...
np.save("deconvolved_f160w.npy", inten)
